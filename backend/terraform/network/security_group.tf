...
resource "aws_security_group" "endpoint" {
  name        = "${var.resource_prefix}_endpoint"
  description = "For VPC Endpoint traffic"
  vpc_id      = module.vpc.vpc_id
  ...
  ingress {
    ...
    to_port         = 443
    protocol        = "tcp"
    security_groups = [aws_security_group.web.id, aws_security_group.tsdb.id, aws_security_group.lambda.id]
  }
