...
class DataManager : BaseManager {
    ...
    func getAnalyticsAggregated(formula: String, aggregateFunction : AggregateFunction, startDate: Date, endDate: Date, callback: @escaping ((Double?, Error?) -> Void)) -> Void {
        let requestData = AnalyticsAggregatedRequest(start_time: Int64(startDate.timeIntervalSince1970 * 1000), end_time: Int64(endDate.timeIntervalSince1970 * 1000), formula: formula, agg_function: aggregateFunction)
